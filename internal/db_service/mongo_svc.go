...
func NewMongoService[DocType interface{}](config MongoServiceConfig) DbService[DocType] {
	...
	if svc.ServerHost == "" {
		svc.ServerHost = enviro("PROJECT_API_MONGODB_HOST", "localhost")
	}

	if svc.ServerPort == 0 {
		port := enviro("PROJECT_API_MONGODB_PORT", "27017")
	...
	if svc.UserName == "" {
		svc.UserName = enviro("PROJECT_API_MONGODB_USERNAME", "")
	}

	if svc.Password == "" {
		svc.Password = enviro("PROJECT_API_MONGODB_PASSWORD", "")
	}

	if svc.DbName == "" {
		svc.DbName = enviro("PROJECT_API_MONGODB_DATABASE", "gregor-hires-project-evidence")
	}

	if svc.Collection == "" {
		svc.Collection = enviro("PROJECT_API_MONGODB_COLLECTION", "project")
	}

	if svc.Timeout == 0 {
		seconds := enviro("PROJECT_API_MONGODB_TIMEOUT_SECONDS", "10")
